...
class logger (object) :
    ...
    def __init__(self, log_file=None, task=None, debug_level=0, show_time=True) :
        ...
        self.write("{t} ===== {task} START >>>>> `{log}`".format(task=self._task, log=log_file, t=tstr), -1)
    ...
    def __repr__(self) :
        return ("{t} Logger level {d} at file `{f}`".format(t=self._task, f=self._file, d=self._debug))
